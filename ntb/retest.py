...
if True == True:    
    ...
    for x in loaders['test']:
        print(x['out_sid'].shape)
        print(x['out_rating'].shape)
        print(x['relevance'].shape)
        break
    # In[15]:
    ...
    if exp_found:
        ...
        model = easy_torch.process.load_model(cfg["model"], model_path, main_module=main_module, map_location=torch.device('cpu'))
